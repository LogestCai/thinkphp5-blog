...
function F_S($str,$value=null,$expir=null){
    if($value===null)
    {
        //获取缓存
        ...
        //设置缓存
        Cache::set($str,$value,$expir);
    }
}

function F_exceptHtmlTags($str,$length='100')
{
    $str =  strip_tags($str);
...
function F_getTree0( $array)//已废弃
{
    ...
    foreach($items as $key => $value)
    {
        ...
        {
            //把当前的$value放到pid节点的children中 注意 这里传递的是引用 为什么呢？
...
function F_getTree ($data, $pid = 0) {
    $tree = [];
    foreach($data as $value){
            if($value['pid'] == $pid){
                $value['children'] = F_getTree($data,$value['id']);
 ...
 *
 * */
function F_getTreeWithoutChild( $array, $pid=0, $level = 0 )
{
